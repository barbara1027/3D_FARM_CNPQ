...
export class QualidadeImpressaoController {
        ...
        if (Number.isNaN(id)) {
          ...
          if (!qualidade) {
            return res.status(404).json({
              message: "Qualidade não encontrada.",
            });
          }
  ...
  criar = async (req: Request, res: Response) => {
      try {
        ...
        if (altura === undefined || espessura === undefined || preenchimento === undefined || velocidade === undefined || temperaturaBico === undefined || temperaturaMesa === undefined || suporte === undefined || adesao === undefined) {
            return res.status(400).json({message:"Os campos altura, espessura, preenchimento, velocidade, temperaturaBico, temperaturaMesa, suporte e adesao são obrigatórios.",});
        }
  ...
  atualizar = async (req: Request, res: Response) => {
    try {
      ...
      const statusCode =
        error.message === "Qualidade não encontrada." ? 404 : 400;
